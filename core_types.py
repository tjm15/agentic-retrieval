# core_types.py
# (This should be the complete version from the "Reproduce full updated code" response,
# ensuring ReasoningNode has all metadata like node_type_tag, generic_material_considerations, etc.)
# For brevity, I'm assuming the full, correct version is here.
# Key elements: IntentStatus, RetrievalSourceType, ProvenanceLog, RetrievedItem, Intent, ReasoningNode
# Ensure Intent takes llm_policy_context_summary and ReasoningNode has all the new metadata fields.
import uuid
import json
import time
...
class ProvenanceLog:
    ...
    def complete(self, status: str, result_summary: Optional[Any] = None):
        ...
        # duration = self.duration(); duration_str = f"{duration:.2f}s" if duration is not None else "N/A"
        # print(f"Provenance: {self.task_description} ({self.intent_id or 'NodeLog'}) -> {status}. Duration: {duration_str}") # Optional: move print to MRM
...
class RetrievedItem: # Same as before
...
class Intent: # Same as before, including llm_policy_context_summary
...
class ReasoningNode: # Includes new generic metadata fields
    ...
    def add_sub_node(self, sub_node: 'ReasoningNode'): # Same
    ...
    def get_sub_node_output_by_key(self, sub_node_key: str) -> Optional[Dict]: # Same
        ...
        return None
    def update_status_based_on_children_and_intents(self): # Same (includes confidence aggregation stub)
        if self.sub_nodes:
            ...
            elif self.intents_issued: self.confidence_score = self.intents_issued[-1].confidence_score
        elif self.intents_issued:
            last_intent = self.intents_issued[-1]
            self.status = last_intent.status
